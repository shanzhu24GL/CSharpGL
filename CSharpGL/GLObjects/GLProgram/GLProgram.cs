...
namespace CSharpGL {
    ...
    public unsafe partial class GLProgram {
        ...
        public static (GLProgram?, string log) Create(string[] feedbackVaryings, BufferMode mode, params Shader[] shaders) {
            ...
            var (program, log) = TryCompile(programId, shaders);
        ...
        /// </summary>
        /// <param name="shaders"></param>
        public static (GLProgram?, string log) Create(params Shader[] shaders) {
            var gl = GL.current; if (gl == null) { return (null, "openGL not ready"); }
            GLuint programId = gl.glCreateProgram();
        ...
        private static (GLProgram?, string log) TryCompile(GLuint programId, params Shader[] shaders) {
            var gl = GL.current; if (gl == null) { return (null, "openGL not ready"); }
            //if (shaders.Length < 1) { throw new ArgumentException(); }

            foreach (Shader item in shaders) {
                gl.glAttachShader(programId, item.shaderId);
            }
